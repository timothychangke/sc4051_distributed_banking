...
2. **Build the C++ client binary:**
   ...
   # Output should be at ./bin/client (or wherever the Makefile puts it)
   ```
...
→ The client's output format for account numbers might be different. Check `extractAccountNumber()` and adjust the parsing logic.
...
→ Make sure the server prints the mode string to stdout or stderr on startup. The test looks for the substring "at-least-once" in the combined output.

**Monitor tests failing**
→ Monitor tests launch two separate client processes. Make sure the client binary doesn't conflict when multiple instances run simultaneously on the same machine.
